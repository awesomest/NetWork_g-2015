...
public:
	...
	//+++++++++++++++++++++++++++++++++
	NEIGHBER_TYPE neighber;//隣接頂点集合(重さ)
	VERTEX();//コンストラクタ
...
inline void VERTEX::show_neighber()
{
	...
	NEIGHBER_TYPE::iterator pneighber = neighber.begin();
	...
	void calcClasta()
	{
		...
		for(register int i = 0;i < vertex.size();i++)
		{
			if(vertex[i].degree > 1)
			{
				//計算
				int triangle = 0;
				NEIGHBER_TYPE::iterator pm1 = vertex[i].neighber.begin();
				NEIGHBER_TYPE::iterator neighberEnd = vertex[i].neighber.end();
				for(;pm1 != neighberEnd; pm1++){
					NEIGHBER_TYPE::iterator pm2 = pm1;
					NEIGHBER_TYPE::iterator judge = vertex[*pm1].neighber.end();
					for(++pm2;pm2 != neighberEnd; pm2++){
						if(vertex[*pm1].neighber.find(*pm2) != judge){
							triangle++;
						}
					}
				}
...
int main(void){
	...
	while(e > 0)
	{
		...
		net.vertex[v1].neighber.insert(v2);
		net.vertex[v2].neighber.insert(v1);
		e--;
	}
