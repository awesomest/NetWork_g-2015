...
public:
	...
	//+++++++++++++++++++++++++++++++++
	map<int,int> neighber;//隣接頂点集合(重さ)
	VERTEX();//コンストラクタ
...
inline void VERTEX::show_neighber()
{
	...
	map<int,int>::iterator pneighber = neighber.begin();
	...
	void calcClasta()
	{
		...
		for(register int i = 0;i < vertex.size();i++)
		{
			if(vertex[i].degree > 1)
			{
				//計算
				int triangle = 0;
				map<int,int>::iterator pm1 = vertex[i].neighber.begin();
				for(;pm1 != vertex[i].neighber.end();pm1++){
					map<int,int>::iterator pm2 = pm1;
